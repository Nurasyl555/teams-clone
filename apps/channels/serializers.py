...
class CreateChannelSerializer(ModelSerializer):
    ...
    name = CharField(min_length=1, max_length=200, trim_whitespace=True)

    description = CharField(required=False, allow_blank=True, max_length=1000)
    ...
    def validate(self, attrs):
        ...
        if Channel.objects.filter(team=team, name__iexact=name).exists():
            logger.warning(
                "Channel creation failed - name exists: team=%s name=%s", team.id, name
            )
...
class UpdateChannelSerializer(ModelSerializer):
    ...
    name = CharField(min_length=1, max_length=200, trim_whitespace=True, required=True)
    ...
    def validate_name(self, value: str) -> str:
        ...
        qs = Channel.objects.filter(team=channel.team, name__iexact=value).exclude(
            pk=channel.pk
        )
    ...
    def update(self, instance: Channel, validated_data: dict) -> Channel:
        ...
        logger.info("Channel updated: id=%s name=%s", instance.id, instance.name)
...
class AddChannelMemberSerializer(ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if ChannelMembership.objects.filter(channel=channel, user=user).exists():
            logger.warning(
                "Membership already exists: channel=%s user=%s", channel.id, user.id
            )
            raise ValidationError({"user": "User is already a member of this channel"})
