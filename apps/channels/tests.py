...
class ChannelTests(APITestCase):
    def setUp(self):
        ...
        self.team_owner = CustomUser.objects.create_user(
        ...
        TeamMembership.objects.create(
            team=self.team, user=self.team_owner, role="admin"
        )
        TeamMembership.objects.create(
            team=self.team, user=self.team_member, role="member"
        )

        # 3. Создаем публичный и приватный каналы
        self.public_channel = Channel.objects.create(
            name="general", team=self.team, is_private=False
        )
        self.private_channel = Channel.objects.create(
            name="secret", team=self.team, is_private=True
        )

        # 4. Добавляем владельца в приватный канал
        ChannelMembership.objects.create(
            channel=self.private_channel, user=self.team_owner
        )
    ...
    def test_add_member_success(self):
        ...
        response = self.client.post(
            self.get_members_url(self.private_channel.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_channel_list_caching(self):
        ...
        with self.assertNumQueries(
            4
        ):  # Число 6 может отличаться в зависимости от вашего кода, подгоните его, посмотрев, сколько запросов идет реально
        ...
        with self.assertNumQueries(
            0
        ):  # В идеале 0, если пользователь тоже закэширован, или 1-2 на проверку сессии/юзера.
