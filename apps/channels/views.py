...
class ChannelViewSet(ViewSet):
    ...
    def get_queryset(self):
        ...
        qs = (
            Channel.objects.select_related("team")
            .prefetch_related("members", "channel_messages")
            .annotate(
    ...
    def get_channel_or_404(self, pk: int) -> tuple[Channel | None, Response | None]:
        try:
            ...
            return None, Response(
                {"error": "Channel not found."}, status=HTTP_404_NOT_FOUND
            )
    ...
    def list(self, request: Request) -> Response:
        ...
        """
        team_id = request.query_params.get("team") or request.query_params.get(
            "team_id"
        )
    ...
    def retrieve(self, request: Request, pk: int = None) -> Response:
        ...
        if cached_channel_data:
            logger.info(
                "Channel retrieved from CACHE: id=%s by user=%s", pk, request.user.id
            )
            return Response(
                {
        ...
        logger.info("Channel retrieved from DB: id=%s by user=%s", pk, request.user.id)
    ...
    def partial_update(self, request: Request, pk: int = None) -> Response:
        ...
        if not serializer.is_valid():
            logger.warning(
                "Channel update failed: id=%s errors=%s", pk, serializer.errors
            )
            return Response({"error": serializer.errors}, status=HTTP_400_BAD_REQUEST)
        ...
        logger.info("Channel updated: id=%s by user=%s", channel.id, request.user.id)
    ...
    def destroy(self, request: Request, pk: int = None) -> Response:
        ...
        return Response(
            {"message": "Channel deleted successfully"}, status=HTTP_204_NO_CONTENT
        )
    ...
    def _list_members(self, request: Request, channel: Channel) -> Response:
        ...
        memberships = ChannelMembership.objects.filter(channel=channel).select_related(
            "user"
        )
    ...
    def _add_member(self, request: Request, channel: Channel) -> Response:
        ...
        data = {**request.data, "channel": channel.id}

        serializer = AddChannelMemberSerializer(data=data, context={"request": request})
        ...
        logger.info(
            "Channel member added: channel=%s user=%s", channel.id, membership.user.id
        )
    ...
    def _remove_member(self, request: Request, channel: Channel) -> Response:
        ...
        if not user_id:
            return Response(
                {"error": "user_id is required."}, status=HTTP_400_BAD_REQUEST
            )
        ...
        logger.info("Channel member removed: channel=%s user=%s", channel.id, user_id)

        return Response(
            {"message": "Member removed successfully"}, status=HTTP_204_NO_CONTENT
        )
