...
@extend_schema_view(
    ...
    create=extend_schema(
        ...
        responses={
            201: OpenApiResponse(
                response=TeamSerializer, description="Team created successfully"
            ),
    ...
    partial_update=extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                response=TeamSerializer, description="Team updated successfully"
            ),
            400: OpenApiResponse(description="Validation error"),
            404: OpenApiResponse(description="Team not found"),
        },
...
class TeamViewSet(ViewSet):
    ...
    def get_team_or_404(self, pk: int) -> tuple[Team | None, Response | None]:
    ...
    def partial_update(self, request: Request, pk: int = None) -> Response:
        ...
        if not serializer.is_valid():
            logger.warning("Team update failed: id=%s errors=%s", pk, serializer.errors)
            return Response(
                {"error": serializer.errors},
                status=HTTP_400_BAD_REQUEST,
            )
    ...
    @action(detail=True, methods=["get", "post"], url_path="assigment")
    def assigments(self, request: Request, pk: int = None) -> Response:
        """
    ...
    def _list_assigments(self, request: Request, team: int = None) -> Response:
        """
        List assigments
        """
        assigment = Assignments.objects.filter(team=team).order_by("due_data")

        serializer = AssigmentsSerialzers(assigment, many=True)

        logger.info(
            "List assigments by team and team_id: %s,assigment:%s", team, assigment
        )

        return Response(
            {"meassage": "Assigments by team_id", "data": serializer.data},
            status=HTTP_200_OK,
        )

    def _create_assignment(self, request: Request, team: Team) -> Response:
    ...
    def _list_members(self, request: Request, team: Team) -> Response:
        """List of members"""

        filtering = build_membership_q(request, team)
        memberships = (
            TeamMembership.objects.filter(filtering)
            .select_related("user")
            .prefetch_related("members")
        )

        serializer = TeamMembershipSerializer(memberships, many=True)
        logger.info("Listed members: team=%s count=%s", team.id, memberships.count())
    ...
    def _add_members(self, request: Request, team: Team) -> Response:
        """Add member to team"""
        data = {**request.data, "team": team.id}
        serializer = CreateTeamMembershipSerializer(
            data=data, context={"request": request}
        )
        if not serializer.is_valid():
            logger.warning(
                "Add member failed: team=%s errors=%s", team.id, serializer.errors
            )
            return Response(
                {"error": serializer.errors},
                status=HTTP_400_BAD_REQUEST,
    ...
    def _delete_members(self, request: Request, team: Team) -> Response:
