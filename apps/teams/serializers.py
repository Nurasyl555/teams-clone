...
class TeamSerializer(ModelSerializer):
    ...
    def get_owner_info(self, obj: Team) -> dict:
        owner = obj.owner
        return {
            "id": owner.id,
...
class CreateTeamSerializer(ModelSerializer):
    ...
    name = CharField(min_length=2, max_length=100, trim_whitespace=True)
    ...
    def create(self, validated_data: dict) -> Team:
        ...
        logger.info(
            "User %s creating team: %s", request.user.id, validated_data.get("name")
        )
        return Team.objects.create(owner=request.user, **validated_data)
...
class UpdateTeamSerializer(ModelSerializer):
    ...
    name = CharField(min_length=2, max_length=100, trim_whitespace=True, required=False)
    description = CharField(required=False, allow_blank=True, max_length=500)
...
class TeamMembershipSerializer(ModelSerializer):
    ...
    def get_role_display(self, obj: TeamMembership) -> str:
...
class CreateTeamMembershipSerializer(ModelSerializer):
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        if TeamMembership.objects.filter(team=team, user=user).exists():
            logger.warning(
                "Membership already exists: user=%s team=%s", user.id, team.id
            )
            raise ValidationError({"error": "User is already a member of this team."})
...
class UpdateTeamMembershipSerializer(ModelSerializer):
    ...
    def update(self, instance: TeamMembership, validated_data: dict) -> TeamMembership:
        """Update team member"""
